...
function byteLength(tx: TransactionBase, txSpecific: ZcashSpecific) {
...
function toBuffer(
    tx: TransactionBase,
    txSpecific: ZcashSpecific,
    buffer?: Buffer,
    initialOffset?: number,
) {
    if (!buffer) buffer = Buffer.allocUnsafe(byteLength(tx, txSpecific));
...
function getExtraData(tx: TransactionBase) {
...
function fromBuffer(buffer: Buffer, options: TransactionOptions) {
    ...
    const txSpecific: ZcashSpecific = {
        joinsplits: [],
    ...
    if (tx.version >= ZCASH_JOINSPLITS_SUPPORT_VERSION) {
        ...
        for (let i = 0; i < joinSplitsLen; ++i) {
            ...
            txSpecific.joinsplits.push({
                // @ts-ignore vpubOld is a number
                vpubOld,
                // @ts-ignore vpubNew is a number
                vpubNew,
                anchor,
                nullifiers,
                commitments,
...
export function fromZcashBuffer(buffer: Buffer, options: TransactionOptions) {
