...
interface DashSpecific {
    extraPayload?: Buffer | undefined;
}

function fromBuffer(buffer: Buffer, options: TransactionOptions) {
    ...
    for (let i = 0; i < voutLen; ++i) {
        tx.outs.push({
            value: bufferReader.readUInt64(options.useStringValues),
            script: bufferReader.readVarSlice(),
        });
    }
    ...
    if (tx.version >= 3 && tx.type !== DASH_NORMAL) {
        txSpecific.extraPayload = bufferReader.readVarSlice();
    }

    if (options.nostrict) return { tx, txSpecific };
    if (bufferReader.offset !== buffer.length) throw new Error('Transaction has unexpected data');

    return { tx, txSpecific };
}

function byteLength(tx: TransactionBase, txSpecific: DashSpecific, _ALLOW_WITNESS = true) {
    ...
    return (
        ...
        (txSpecific.extraPayload ? varSliceSize(txSpecific.extraPayload) : 0) +
...
function toBuffer(
    ...
    if (txSpecific.extraPayload) bufferWriter.writeVarSlice(txSpecific.extraPayload);
...
function getExtraData(tx: TransactionBase, txSpecific: DashSpecific) {
    if (!txSpecific.extraPayload) return;
    const extraDataLength = varuint.encode(txSpecific.extraPayload.length);
    return Buffer.concat([extraDataLength, txSpecific.extraPayload]);
}

export function fromDashBuffer(buffer: Buffer, options: TransactionOptions) {
    const { tx, txSpecific } = fromBuffer(buffer, options);
    tx.byteLength = byteLength.bind(null, tx, txSpecific);
    tx.toBuffer = toBuffer.bind(null, tx, txSpecific);
    tx.getExtraData = getExtraData.bind(null, tx, txSpecific);
    return tx;
}
