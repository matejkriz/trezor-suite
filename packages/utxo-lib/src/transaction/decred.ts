...
function fromBuffer(buffer: Buffer, options: TransactionOptions) {
    ...
    for (let i = 0; i < voutLen; i++) {
        const value = bufferReader.readUInt64(options.useStringValues);
...
export function fromDecredBuffer(buffer: Buffer, options: TransactionOptions) {
    const tx = fromBuffer(buffer, options);
